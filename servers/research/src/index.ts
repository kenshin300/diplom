...
async function getLoginomCache(): Promise<LoginomCache | null> {
  try {
...
async function saveLoginomCache(data: Record<string, string>): Promise<void> {
  const cache: LoginomCache = { fetched_at: new Date().toISOString(), data };
...
function extractStructured(html: string): string {
  ...
  const add = (prefix: string, text: string) => {
    ...
    if (clean.length < 8 || seen.has(clean)) return;
    seen.add(clean);
...
server.registerTool(
  ...
  {
    description:
      "Fetch a web page and return all internal links found on it. Use this to discover which pages of a site are worth reading.",
    inputSchema: {
...
server.registerTool(
  ...
  {
    description:
      "Fetch a web page and return its visible text content. Use this to read about pages, services, and contacts.",
    inputSchema: {
      url: z.string().describe("URL of the page to fetch"),
    },
  },
...
server.registerTool(
  ...
  {
    description:
      "Fetch financial data for a company from checko.ru using its INN (tax ID). " +
...
server.registerTool(
  ...
  {
    description:
      "Return information about Loginom Company (our company) from cache or by fetching loginom.ru. " +
